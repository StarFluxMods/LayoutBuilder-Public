using GUINodeEditor;
using Kitchen.Layouts;
using Kitchen.Layouts.Modules;
using KitchenLib.Preferences;
using UnityEngine;
using LayoutBuilder.Utility;

namespace LayoutBuilder.Editors.Nodes
{
    public class Node_SplitRooms : Node, IXNodeConvertable, ISerializableNode
    {

        public override void Init(Vector2 position)
        {
            Init(position, new Node_SplitRoomsWindow(), title: "SplitRooms");

            AddInput(typeof(LayoutGraphConnection), "Input");
            AddOutput(typeof(LayoutGraphConnection), "Output");
        }

        public void LoadValuesFromXNode(XNode.Node node)
        {
            if (node is SplitRooms roomGrid)
            {
                FixSeed = roomGrid.FixSeed;
                UniformX = roomGrid.UniformX;
                UniformY = roomGrid.UniformY;
                RandomX = roomGrid.RandomX;
                RandomY = roomGrid.RandomY;
            }
        }

        public T ConvertToXNode<T>() where T : XNode.Node, new()
        {
            T node = (T)ScriptableObject.CreateInstance(typeof(T));
            if (node is SplitRooms roomGrid)
            {
                roomGrid.FixSeed = FixSeed;
                roomGrid.UniformX = UniformX;
                roomGrid.UniformY = UniformY;
                roomGrid.RandomX = RandomX;
                roomGrid.RandomY = RandomY;

                roomGrid.position = nodeWindow.rect.position;
            }

            return node;
        }

        public int FixSeed;
        public int UniformX;
        public int UniformY;
        public int RandomX;
        public int RandomY;
    }

    public class Node_SplitRoomsWindow : NodeModule
    {
        public override float GetWindowWidth()
        {
            return 160;
        }

        public override void OnGUI()
        {

            Node_SplitRooms n = (Node_SplitRooms)node;
            backgroundColor = Utils.MenuColorToColor[Mod.Manager.GetPreference<PreferenceInt>("MENU_COLOR").Value];
            
            CreateTopDocks(n.outputs[0], n.inputs[0]);
            
            CreateInt("FixSeed", ref n.FixSeed);
            CreateInt("UniformX", ref n.UniformX);
            CreateInt("UniformY", ref n.UniformY);
            CreateInt("RandomX", ref n.RandomX);
            CreateInt("RandomY", ref n.RandomY);
        }
    }
}